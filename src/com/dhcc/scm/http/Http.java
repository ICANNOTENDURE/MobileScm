...
import com.dhcc.scm.http.Request.HttpMethod;
...
public class Http {
	...
	public static class Builder {
		...
		public void request() {
			request(new Http(httpConfig));
		}

		public void request(Http kjHttp) {
			switch (httpMethod) {
			case HttpMethod.GET:
	 ...
	 *            是否缓存本条请求
	 */
	public Request<byte[]> get(String url, HttpParams params, boolean useCache, HttpCallBack callback) {
		if (params != null) {
			url += params.getUrlParams();
		}
		Request<byte[]> request = new FormRequest(HttpMethod.GET, url, params, callback);
		request.setShouldCache(useCache);
		doRequest(request);
		return request;
	}
	 ...
	 *            是否缓存本条请求
	 */
	public Request<byte[]> jsonPost(String url, HttpParams params, boolean useCache, HttpCallBack callback) {
		Request<byte[]> request = new JsonRequest(HttpMethod.POST, url, params, callback);
		request.setShouldCache(useCache);
		doRequest(request);
		return request;
	}
	...
	public Request<byte[]> jsonGet(String url, HttpParams params, HttpCallBack callback) {
		Request<byte[]> request = new JsonRequest(HttpMethod.GET, url, params, callback);
		doRequest(request);
		return request;
	}
	 ...
	 *            是否缓存本条请求
	 */
	public Request<byte[]> jsonGet(String url, HttpParams params, boolean useCache, HttpCallBack callback) {
		Request<byte[]> request = new JsonRequest(HttpMethod.GET, url, params, callback);
		request.setShouldCache(useCache);
		doRequest(request);
		return request;
	...
	public void resumeTask(String storeFilePath, String url) {
		DownloadController controller = mConfig.mController.get(storeFilePath, url);
		controller.resume();
	}
	...
	private void start() {
		...
		mCacheDispatcher = new CacheDispatcher(mCacheQueue, mNetworkQueue, mConfig);
		mCacheDispatcher.start();
		// 构建线程池
		for (int i = 0; i < mTaskThreads.length; i++) {
			NetworkDispatcher tasker = new NetworkDispatcher(mNetworkQueue, mConfig.mNetwork, HttpConfig.mCache, mConfig.mDelivery);
	...
	public <T> Request<T> add(Request<T> request) {
		...
		synchronized (mWaitingRequests) {
			...
			if (mWaitingRequests.containsKey(cacheKey)) {
				...
				if (HttpConfig.DEBUG) {
					Loger.debug("Request for cacheKey=%s is in flight, putting on hold.", cacheKey);
				}
			} else {
		...
		if (request.shouldCache()) {
			synchronized (mWaitingRequests) {
				...
				if (waitingRequests != null) {
					if (HttpConfig.DEBUG) {
						Loger.debug("Releasing %d waiting requests for cacheKey=%s.", waitingRequests.size(), cacheKey);
					}
