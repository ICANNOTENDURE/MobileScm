...
public class DiskCache implements Cache {
    ...
    public synchronized void clean() {
        ...
        mEntries.clear();
        mTotalSize = 0;
        Loger.debug("disk Cache cleared.");
    }
    ...
    public synchronized Entry get(String key) {
        ...
        try {
            ...
            Loger.debug("%s: %s", file.getAbsolutePath(), e.toString());
            remove(key);
            return null;
        } finally {
            if (cis != null) {
    ...
    public synchronized void initialize() {
        if (!mRootDirectory.exists()) {
            if (!mRootDirectory.mkdirs()) {
                Loger.debug("Unable to create cache dir %s",
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        try {
            ...
            if (!success) {
                fos.close();
                Loger.debug("Failed to write header for %s",
        ...
        if (!deleted) {
            Loger.debug("Could not clean up file %s", file.getAbsolutePath());
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        if (HttpConfig.DEBUG) {
            Loger.debug("pruned %d files, %d bytes, %d ms", prunedFiles,
